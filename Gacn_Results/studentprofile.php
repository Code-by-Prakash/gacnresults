...
if (strlen($_SESSION['admin']) == 0) {
  ...
  $currentTime = date('d-m-Y h:i:s A', time());
    ...
    if (isset($_POST['submit'])) {
        ...
        $email = $_POST['email'];
        $mobile = $_POST['mobile'];
        $classid= $_POST['cid'];        // Check if an image was uploaded
        if (isset($_FILES['image']) && !empty($_FILES['image']['name'])) {
            ...
            if (!in_array($extension, $allowed_extensions)) {
                ...
                $query = mysqli_query($con, "UPDATE tblstudents SET `Name`='$name', Userid='$reg', Department='$dept', `Degree & Branch`='$degree',
              DOB='$dob', `Batch Start`='$batchs', `Batch End`='$batche', mobile='$mobile', ClassId='$classid', userImage='$imgnewfile', emailId='$email', updationDate=NOW() WHERE Userid='" . $_GET['uid'] . "'");
            ...
            $query = mysqli_query($con, "UPDATE tblstudents SET `Name`='$name', Userid='$reg', Department='$dept', `Degree & Branch`='$degree',
        DOB='$dob', `Batch Start`='$batchs', `Batch End`='$batche', mobile='$mobile',ClassId='$classid',  emailId='$email', updationDate=NOW() WHERE Userid='" . $_GET['uid'] . "'");
            if ($query) {
                $successmsg = "Profile successfully updated !!";
            } else {
                $errormsg = "Profile not updated !!";
...
<head>
    ...
    <title>editStudentProfile</title>
    <!-- Bootstrap core CSS -->
        ...
        <div class="row mt">
            <div class="col-lg-12">
                <div class="form-panel">
                    ...
                    <?php
                    $query = mysqli_query($con, "SELECT * FROM tblstudents WHERE Userid='" . $_GET['uid'] . "'");
                    while ($row = mysqli_fetch_array($query)) { ?>
                        <h4 class="mb"><i class="fa fa-user"></i>&nbsp;&nbsp;
                            <?php echo htmlentities($row['Name']); ?>'s Profile
                        </h4>
                        ...
                        <form class="form-horizontal style-form" method="post" name="profile" enctype="multipart/form-data">
                            <div class="form-group">
                                ...
                                <div class="col-sm-4">
                                    <input type="text" name="name" placeholder="Enter a student name"required="required" value="<?php echo htmlentities($row['Name']); ?>" class="form-control">
                                </div>
                                <label class="col-sm-2 control-label">Registration Number</label>
                                <div class="col-sm-4">
                                    <input type="number" placeholder="Enter the reg.no" min="1" name="regno" required="required" value="<?php echo htmlentities($row['Userid']); ?>" class="form-control">
                                </div>
                            </div>
                            <div class="form-group">
                            ...
                            <div class="form-group">
                                ...
                                <div class="col-sm-4">
                                    <input type="text" placeholder="Enter the degree and branch" name="degree" required="required" value="<?php echo htmlentities($row['Degree & Branch']); ?>" class="form-control">
                                </div>
                            ...
                            <div class="form-group">
                                ...
                                <div class="col-sm-4">
                                    ...
                                    <input type="number" name="batchs" required="required" value="<?php echo htmlentities($row['Batch Start']); ?>" class="form-control"
                                                placeholder="Select Year" min="1900" max="2099" >
                                </div>
                                <label class="col-sm-2 control-label">Batch End year</label>
                                <div class="col-sm-4">
                                    <input type="number"min="1990" max="2099" name="batche" required="required" value="<?php echo htmlentities($row['Batch End']); ?>" class="form-control">
                                </div>
                            </div>
                            <div class="form-group">
                                <label class="col-sm-2 control-label">Email Id</label>
                                <div class="col-sm-4">
                                    <input type="email" placeholder="Enter the email id"name="email"  value="<?php echo htmlentities($row['emailId']); ?>" class="form-control">
                                </div>
...
<div class="col-sm-4">
    <input type="tel" placeholder="Enter a 10-digit mobile number" pattern="[0-9]{10}" name="mobile" value="<?php echo htmlentities($row['mobile']); ?>" class="form-control" required>
                            ...
                            <div class="form-group">
                                ...
                                <div class="col-sm-4">
                                    <input type="number" min="1" placeholder="Enter the class id" name="cid" required="required" value="<?php echo htmlentities($row['ClassId']); ?>" class="form-control">
                                </div>
                    </div>
                            <div class="form-group">
                                ...
                                <div class="col-sm-4">
                                    ...
                                    if ($userphoto == ""):
                                    ?>
                                        <img src="http://localhost/Gacn_Results/users/userimages/noimage.png" width="256" height="256">
                                    <?php else: ?>
                                        <img src="http://localhost/Gacn_Results/users/userimages/<?php echo htmlentities($userphoto); ?>" width="256" height="256">
                                    <?php endif; ?>
                                </div>
                            </div>
                            <div class="form-group">
