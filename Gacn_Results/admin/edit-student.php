...
if (strlen($_SESSION['admin']) == 0) {
    ...
    {
    $userid=$_GET['uid'];
    $query=mysqli_query($con,"delete from tblstudents where Userid='$userid'");
...
<?php $query=mysqli_query($con,"select * from tblstudents");
$cnt=1;
...
<td><a href="javascript:void(0);" onClick="popUpWindow('http://localhost/Gacn_Results/admin/studentprofile.php?uid=<?php echo htmlentities($row['Userid']);?>');" title="Edit Details">
<button type="button" class="btn btn-primary">Edit Detials</button>
											</a>
<a href="edit-student.php?uid=<?php echo htmlentities($row['Userid']);?>&&action=del" title="Delete" onClick="return confirm('Do you really want to delete ?')">
