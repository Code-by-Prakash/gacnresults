...
if (isset($_POST['exam'])) {
  ...
  $stmt = $con->prepare("SELECT DISTINCT tblclasses.ClassName, tblsubjects.Year
                         FROM tblstudents
  ...
  JOIN tblclasses on tblsubjectcombination.ClassId=tblclasses.ClassId AND tblstudents.ClassId=tblclasses.ClassId JOIN tblstaffs on tblstaffs.Department=tblstudents.Department
                         WHERE DATE_FORMAT(tblresults.MonthYear, '%b %Y') = ? and tblstaffs.Department=tblstudents.Department");
