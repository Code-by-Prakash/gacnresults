...
if (isset($_POST['exam']) && isset($_POST['batch']) && isset($_POST['class']) && isset($_POST['year']) ) {
  $exam = $_POST['exam'];
  $batch = $_POST['batch'];
  $years = explode('-', $batch);
  $batchStart = $years[0];
...
$staffid = $row['StaffId'];
  ...
  $sql = "SELECT DISTINCT tblsubjects.SubjectName, tblresults.SubjectId
          FROM tblstudents
          JOIN tblresults ON tblresults.StudentId = tblstudents.Userid 
          JOIN tblsubjects ON tblresults.SubjectId = tblsubjects.SubjectId 
          JOIN tblsubjectcombination ON tblsubjects.SubjectId = tblsubjectcombination.SubjectId
          ...
          AND tblstudents.`Batch End` = '" . $batchEnd . "' and tblclasses.ClassName='".$class."' and tblsubjects.Year='".$year."'
